...
# Initialize the accelerator with FSDP
...
def register_hooks(model, modified_neurons_layers, scale_factors, accelerator):
    unwrapped_model = accelerator.unwrap_model(model)
    hooks = []
    for layer_idx, feature_indices in modified_neurons_layers.items():
        scale_factor = scale_factors[layer_idx]
        hook = FeatureScalerHook(unwrapped_model, layer_idx, feature_indices, scale_factor, accelerator)
        hooks.append(hook)
    return hooks
...
class FeatureScalerHook:
    def __init__(self, model, layer_idx, feature_indices, scale_factor, accelerator):
        self.model = model
        self.layer_idx = layer_idx
        self.feature_indices = feature_indices
        self.scale_factor = scale_factor
        self.accelerator = accelerator
        self.register_individual_hook()

    def register_individual_hook(self):
        # Access the specific layer
        target_layer = self.model.visual.transformer.resblocks[self.layer_idx].mlp.c_fc
...
# Define your datasets and dataloaders as before
dataset1 = ImageTextDataset("path/to/images/COCO/data-square", 
                            "path/to/COCO/data-square/short-coco-sprite-train-0_9.json", 
...
val_dataset = ImageTextDataset("path/to/images/COCO/data-square", 
                               "path/to/COCO/data-square/short-coco-sprite-val-10_11.json", 
                               transform=preprocess)
...
optimizer = AdaBelief(param_groups, lr=learning_rate, eps=1e-14, betas=(0.9, 0.999), 
                      weight_decay=1e-3, weight_decouple=True, rectify=True, print_change_log=False)

scheduler = OneCycleLR(optimizer, max_lr=max_learning_rate, total_steps=total_steps, 
...
print("== START == \n")
print(register_hooks.__code__.co_argcount)  # Should output 4
print(FeatureScalerHook.__init__.__code__.co_argcount)  # Should output 6 (including self)
...
def trainloop():
    ...
    # Register hooks with the updated function definition
    hooks = register_hooks(model, modified_neurons_layers, scale_factors, accelerator)
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (images, texts) in progress_bar:
            ...
            # Logging and gradient monitoring
            ...
            progress_bar.set_postfix({
                'loss': f'{total_train_loss / (batch_idx + 1):.4f}  --  Logits Image: {logits_per_image.mean().item():.3f}, Text: {logits_per_text.mean().item():.3f}'
            })

        avg_train_loss = total_train_loss / len(train_dataloader)
        training_losses.append(avg_train_loss)

        logits_images.append(torch.mean(torch.stack([log.mean() for log in logits_texts])).item())
        logits_texts.append(torch.mean(torch.stack([log.mean() for log in logits_images])).item())
        ...
        with open(f"{text_logs_folder}/log_details_train.txt", "a", encoding='utf-8') as f:
            f.write(f"Epoch {epoch + 1}/{EPOCHS}, Loss: {avg_train_loss:.4f}, " +
                    f"Training Acc: {epoch_train_acc:.4f}, Training F1: {epoch_train_f1:.4f}\n")
        ...
        print(Fore.YELLOW + f"Epoch {epoch + 1}/{EPOCHS} - Validation Acc: {epoch_val_acc:.4f}, " +
              f"Validation F1: {epoch_val_f1:.4f}")
        print(Fore.YELLOW + f"Epoch {epoch + 1}/{EPOCHS} - Training Loss: {avg_train_loss:.4f}, " +
              f"Validation Loss: {avg_val_loss:.4f}")
        print(Fore.YELLOW + "============================================================" + Style.RESET_ALL)

        with open(f"{text_logs_folder}/log_training.txt", "a", encoding='utf-8') as f:
            f.write("======================== STATS =============================\n")
            f.write(f"Epoch {epoch + 1}/{EPOCHS} - Validation Acc: {epoch_val_acc:.4f}, " +
                    f"Validation F1: {epoch_val_f1:.4f}\n")
